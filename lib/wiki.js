...
export async function getWikiLocationsData() {
  ...
  for (const location of updatedArray) {
    ...
    updatedPageContent = updatedPageContent + "<a href=" + location.path + " target='_blank' rel='noopener'>Read More...</a>"
...
export async function getOurLocation(id) {
  const { data } = await client.query({
...
export async function getPageContent(id) {
  ...
  const updatedDetails = details.replace(/href/g, 'target="_blank" rel="noreferrer" href');
