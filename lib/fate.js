...
export const getAccount = async (accountId) => await getAccountData(accountId, accountQuery);
...
export async function getLocationsData(query) {
  ...
  for (const location of data.locations) {
    if (location.characters.length) {
      ...
      locations.push({
        name: location.name,
        known: _.orderBy(known, ['is17', 'isDecrepit', 'timestamp'], ['desc', 'desc', 'asc']),
...
export async function getAccountData(accountId, query) {
  ...
  const nfts = await Promise.all(data.account.characters.map(async character => {
    const response = await fetch(`https://fateofwagdie.com/api/characters/${character.id}`);
    const data = await response.json();
  ...
  return _.chain(locations)
    .map((l) => {
      return {
        name: l.name,
        known: _.orderBy(l.known, ['is17', 'isDecrepit', 'timestamp'], ['desc', 'desc', 'asc']),
