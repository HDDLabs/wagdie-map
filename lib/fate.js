...
export async function getLocationsData(query) {
  ...
  for (const location of data.locations) {
    if (location.characters.length) {
      ...
      const known = nfts.filter(nft => nft.hasName);
      const unknown = nfts.filter(nft => !nft.hasName);

      locations.push({
        name: location.name,
        known: _.orderBy(known, ['is17', 'isDecrepit', 'isRoyal', 'isSilence'], ['desc', 'desc', 'desc', 'desc']),
        unknown: _.orderBy(unknown, ['is17', 'isDecrepit', 'isRoyal', 'isSilence'], ['desc', 'desc', 'desc', 'desc']),
      });
    }
  }
